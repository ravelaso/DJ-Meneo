...
module.exports = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (inputType === urlType.youtubeVideo) {
        ...
        } else if (inputType === urlType.youtubeShorts) {
            Logger.LogMessage("Play Youtube Shorts Command Received!");
            const yt = await YouTube.getVideo(guildPlayer.convertShortsToWatch(input));
            await guildPlayer.addToQueue(
                input,
                audioType.Youtube,
                yt.title,
            ...
            Logger.LogMessage("Searching Youtube for input.");
            let yt = await YouTube.searchOne(input);
            await guildPlayer.addToQueue(
                yt.url,
                audioType.Youtube,
                yt.title,
